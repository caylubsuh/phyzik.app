...
import { listPayouts, getBrandGmv, getAdminUser } from '@/lib/marketplace/admin'
      ...
      <div className="flex flex-col gap-8">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <StatCard
            ...
            hint="Paid orders, all brands"
            icon={<TrendingUp className="h-4 w-4" />}
            emphasis
          />
          <StatCard
            label="Paid out"
            value={formatCents(totalPaidOut)}
            hint="Settled to brands"
                ...
                {gmv.length === 0 ? (
                  ...
                  gmv.map((b) => (
                    <tr key={b.brandId} className="border-b border-border/50 last:border-b-0">
                      ...
                      <td className="px-4 py-3 font-mono text-[13px] text-text-secondary tabular-nums">
                        {b.orders}
                      </td>
                      <td className="px-4 py-3 font-mono text-[13px] font-medium text-text-primary tabular-nums">
                        {formatCents(b.gmvCents)}
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="overflow-hidden rounded-[3px] border border-border bg-bg-surface">
            <table className="w-full border-collapse text-left">
              ...
              <tbody>
                {payouts.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-[13px] text-text-tertiary">
                  ...
                  payouts.map((p) => (
                    <tr key={p.id} className="border-b border-border/50 last:border-b-0">
                      <td className="px-4 py-3 text-[13.5px] text-text-primary">
                        {p.brandName ?? '—'}
                      </td>
                      <td className="px-4 py-3 font-mono text-[13px] font-medium text-text-primary tabular-nums">
                        {formatCents(p.amount_cents)}
                      </td>
                      <td className="px-4 py-3 text-[13px] capitalize text-text-secondary">
                        {p.status ?? '—'}
                      </td>
