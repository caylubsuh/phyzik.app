...
import { listOrders } from '@/lib/marketplace/admin'
        ...
        <div className="overflow-hidden rounded-[3px] border border-border bg-bg-surface">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-border">
                  {['Order', 'Date', 'Brand', 'Buyer', 'Total', 'Status', ''].map((h, i) => (
                    <th key={h || i} className="px-4 py-3 text-[10.5px] font-bold uppercase tracking-[0.16em] text-text-tertiary">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-b border-border/50 last:border-b-0 hover:bg-white/[0.02]">
                    <td className="px-4 py-3.5">
                      <Link href={`/admin/orders/${o.id}`} className="font-mono text-[12.5px] text-accent-bright tabular-nums hover:underline">
                        {o.id.slice(0, 8).toUpperCase()}
                      </Link>
                    </td>
                    <td className="px-4 py-3.5 text-[13px] text-text-secondary tabular-nums">{fmtDate(o.created_at)}</td>
                    <td className="px-4 py-3.5 text-[13px] text-text-primary">{o.brandName ?? '—'}</td>
                    <td className="px-4 py-3.5 text-[13px] text-text-secondary">{o.ship_to?.name ?? '—'}</td>
                    ...
                    <td className="px-4 py-3.5"><StatusChip kind="order" status={o.status as OrderStatus} /></td>
                    <td className="px-4 py-3.5 text-right">
                      <Link href={`/admin/orders/${o.id}`} className="inline-flex text-text-tertiary hover:text-accent-bright">
